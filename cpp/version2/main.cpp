...
std::string traducir(const std::string& texto) {
    std::string textoCodificado = codificarURL(texto);

    std::string comando =
        "curl -s \"https://translate.googleapis.com/translate_a/single?client=gtx&sl=en&tl=es&dt=t&q=" +
        textoCodificado +
        "\"";

    std::string respuesta = ejecutarComando(comando);

    std::string inicioTag = "[[[\"";
    size_t inicio = respuesta.find(inicioTag);
...
int main() {
    ...
    direccion.sin_port = htons(8131);

    bind(servidor, (sockaddr*)&direccion, sizeof(direccion));
    listen(servidor, 5);

    std::cout << "Servidor C++ version2 en http://localhost:8131/?n=10" << std::endl;
    ...
    while (true) {
        ...
        std::string espanol = traducir(ingles);
